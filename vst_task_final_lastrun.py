...
expName = 'vst_task_final'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Varet\\OneDrive\\Desktop\\VSTMBT_Project\\vst_task_final_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    inst_text = visual.TextStim(win=win, name='inst_text',
        ...
        languageStyle='LTR',
        depth=0.0);
    inst_key = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    Instructions = data.Routine(
        name='Instructions',
        components=[inst_text, inst_key],
    )
    Instructions.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # create starting attributes for inst_key
    inst_key.keys = []
    inst_key.rt = []
    _inst_key_allKeys = []
    ...
    while continueRoutine:
        ...
        # if inst_key is starting this frame...
        if inst_key.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'inst_key.started')
            # update status
            inst_key.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(inst_key.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(inst_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if inst_key.status == STARTED and not waitOnFlip:
            theseKeys = inst_key.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            ...
            if len(_inst_key_allKeys):
                ...
                inst_key.duration = _inst_key_allKeys[-1].duration
    ...
    if inst_key.keys != None:  # we had a response
        ...
        thisExp.addData('inst_key.duration', inst_key.duration)
    thisExp.nextEntry()
    ...
    for thisTrial in trials:
        ...
        # create an object to store info about Routine Sample
        Sample = data.Routine(
            name='Sample',
            components=[fixation, shape_L, shape_C, shape_R],
        )
        Sample.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        shape_L.setColor(color1, colorSpace='rgb')
        shape_L.setImage(shape1)
        shape_C.setColor(color2, colorSpace='rgb')
        shape_C.setImage(shape2)
        shape_R.setColor(color3, colorSpace='rgb')
        shape_R.setImage(shape3)
        # store start times for Sample
        Sample.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        Sample.tStart = globalClock.getTime(format='float')
        Sample.status = STARTED
        thisExp.addData('Sample.started', Sample.tStart)
        Sample.maxDuration = None
        # keep track of which components have finished
        SampleComponents = Sample.components
        for thisComponent in Sample.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "Sample" ---
        thisExp.currentRoutine = Sample
        Sample.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 2.5:
            ...
            # if shape_L is starting this frame...
            if shape_L.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                shape_L.frameNStart = frameN  # exact frame index
                shape_L.tStart = t  # local t and not account for scr refresh
                shape_L.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(shape_L, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'shape_L.started')
                # update status
                shape_L.status = STARTED
                shape_L.setAutoDraw(True)
            
            # if shape_L is active this frame...
            if shape_L.status == STARTED:
                # update params
                pass
            
            # if shape_L is stopping this frame...
            if shape_L.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > shape_L.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    shape_L.tStop = t  # not accounting for scr refresh
                    shape_L.tStopRefresh = tThisFlipGlobal  # on global time
                    shape_L.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'shape_L.stopped')
                    # update status
                    shape_L.status = FINISHED
                    shape_L.setAutoDraw(False)
            
            # *shape_C* updates
            
            # if shape_C is starting this frame...
            if shape_C.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                shape_C.frameNStart = frameN  # exact frame index
                shape_C.tStart = t  # local t and not account for scr refresh
                shape_C.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(shape_C, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'shape_C.started')
                # update status
                shape_C.status = STARTED
                shape_C.setAutoDraw(True)
            
            # if shape_C is active this frame...
            if shape_C.status == STARTED:
                # update params
                pass
            
            # if shape_C is stopping this frame...
            if shape_C.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > shape_C.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    shape_C.tStop = t  # not accounting for scr refresh
                    shape_C.tStopRefresh = tThisFlipGlobal  # on global time
                    shape_C.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'shape_C.stopped')
                    # update status
                    shape_C.status = FINISHED
                    shape_C.setAutoDraw(False)
            
            # *shape_R* updates
            
            # if shape_R is starting this frame...
            if shape_R.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                shape_R.frameNStart = frameN  # exact frame index
                shape_R.tStart = t  # local t and not account for scr refresh
                shape_R.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(shape_R, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'shape_R.started')
                # update status
                shape_R.status = STARTED
                shape_R.setAutoDraw(True)
            
            # if shape_R is active this frame...
            if shape_R.status == STARTED:
                # update params
                pass
            
            # if shape_R is stopping this frame...
            if shape_R.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > shape_R.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    shape_R.tStop = t  # not accounting for scr refresh
                    shape_R.tStopRefresh = tThisFlipGlobal  # on global time
                    shape_R.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'shape_R.stopped')
                    # update status
                    shape_R.status = FINISHED
            ...
            # has a Component requested the Routine to end?
            if not continueRoutine:
                Sample.forceEnded = routineForceEnded = True
            # has the Routine been forcibly ended?
            if Sample.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in Sample.components:
        ...
        for thisComponent in Sample.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for Sample
        Sample.tStop = globalClock.getTime(format='float')
        Sample.tStopRefresh = tThisFlipGlobal
        thisExp.addData('Sample.stopped', Sample.tStop)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if Sample.maxDurationReached:
            routineTimer.addTime(-Sample.maxDuration)
        elif Sample.forceEnded:
            routineTimer.reset()
        else:
        ...
        Test = data.Routine(
            name='Test',
            components=[test_shape_L, test_shape_C, test_shape_R, key_resp],
        )
        ...
        test_shape_L.setColor(test_color_L, colorSpace='rgb')
        test_shape_L.setImage(shape1)
        test_shape_C.setColor(test_color_C, colorSpace='rgb')
        test_shape_C.setImage(shape2)
        test_shape_R.setColor(color3, colorSpace='rgb')
        ...
        while continueRoutine:
            ...
            # if test_shape_L is starting this frame...
            if test_shape_L.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                test_shape_L.frameNStart = frameN  # exact frame index
                test_shape_L.tStart = t  # local t and not account for scr refresh
                test_shape_L.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(test_shape_L, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'test_shape_L.started')
                # update status
                test_shape_L.status = STARTED
                test_shape_L.setAutoDraw(True)
            
            # if test_shape_L is active this frame...
            if test_shape_L.status == STARTED:
                # update params
                pass
            
            # *test_shape_C* updates
            
            # if test_shape_C is starting this frame...
            if test_shape_C.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                test_shape_C.frameNStart = frameN  # exact frame index
                test_shape_C.tStart = t  # local t and not account for scr refresh
                test_shape_C.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(test_shape_C, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'test_shape_C.started')
                # update status
                test_shape_C.status = STARTED
                test_shape_C.setAutoDraw(True)
            
            # if test_shape_C is active this frame...
            if test_shape_C.status == STARTED:
                # update params
                pass
            
            # *test_shape_R* updates
            
            # if test_shape_R is starting this frame...
            if test_shape_R.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                test_shape_R.frameNStart = frameN  # exact frame index
                test_shape_R.tStart = t  # local t and not account for scr refresh
                test_shape_R.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(test_shape_R, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'test_shape_R.started')
                # update status
                test_shape_R.status = STARTED
                test_shape_R.setAutoDraw(True)
            
            # if test_shape_R is active this frame...
            if test_shape_R.status == STARTED:
                # update params
                pass
    ...
    # --- Run Routine "Summary" ---
    thisExp.currentRoutine = Summary
    ...
    while continueRoutine:
        ...
        # has the Routine been forcibly ended?
        if Summary.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in Summary.components:
